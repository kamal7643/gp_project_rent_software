...
def update_cache(head):
    ...
    wsh.write(0, 6, 'time_rented')
    wsh.write(0, 7, 'time_to_return')
    wsh.write(0, 8, 'payment')
    wsh.write(0, 9, 'password')
    for i in range(len(head.customers)):
        ...
        wsh.write(i+1, 6, head.customers[i].time_car_rented)
        wsh.write(i+1, 7, head.customers[i].time_to_return)
        wsh.write(i+1, 8, head.customers[i].payment)
        wsh.write(i+1, 9, head.customers[i].password)
    final.close()
